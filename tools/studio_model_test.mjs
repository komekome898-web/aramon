/* 「モデルで抜く」をスタジオ本体のページで実際に動かして精度と落ち方を確かめる(開発用)。
   ...
   使い方:
     ...
     node tools/studio_model_test.mjs --only iou      項目を絞る(iou / e2e / cache / fallback)
...
/* ページが MODEL_SRC へ出した通信を手元のファイルへ振り替える。
   `blockModel` を真にすると**モデルの取得だけを失敗させる**(落ち方の検査)。 */
async function openStudio(opts = {}){
  ...
  await page.route('**://cdn.jsdelivr.net/npm/onnxruntime-web@**', async route=>{
