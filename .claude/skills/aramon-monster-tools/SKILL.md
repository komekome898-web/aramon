...
- **「覚醒スキンを作る」(`regKind='awaken'`)は元のSSRスキンから覚醒後の姿を作る。** 画像名は元のidから機械的に決める(`guts_ssr` → `guts_ssr_awake` / `guts_player_ssr_awake` / `guts_ssr_awake_walk_f1..8・b1..8`)ので入れ先を間違えない。**元にできるのは`awakenOf`が付いていないSSRだけ**(覚醒後をさらに覚醒させない)。
  - **エフェクト合成(おすすめ)**: 元スキンの18枚を`getRepoImage()`でリポジトリから取り、`awakenComposeOne()`で全部に同じ加工を掛ける。中心は**アルファから作るリムライト+外側グロー**(`awakenRimCanvas()`が16方向へずらして描いた合成から元のシルエットを抜く)で、どんな絵でも破綻せず「力を纏った」見た目になる。色相/彩度/明度のシフトと、任意の素材PNGの加算合成を足せる。**絵は1枚も描かなくてよい。** 歩行コマの無い元スキンなら静止画2枚だけを作る。
  - **描き下ろし**: 覚醒後の絵を渡して既存のSSR追加と同じ歩行生成に通す(`aw_mode='draw'`のときだけ歩行・静止画パネルが出る)。
