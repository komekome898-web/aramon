...
- **入口は2つ**: ロビー左メニューの「🐉レイド」と、プレイモードの「レイドバトル」タブ。**どちらも`raidPlayable(アカウント名)`で出し分け**、文言は`raidPhase()`で作る。進む前の門番は`raidGuardReady()`1か所(モンスター未選択・期間外はトーストを出してロビーへ戻す。**押しても何も起きない状態を作らない**)。
