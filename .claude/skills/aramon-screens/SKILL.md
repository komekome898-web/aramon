...
## バッグ(`#bagOverlay`。アイテム / 称号の2タブ)

- **箱の高さを先に決め、中の欄がそれぞれスクロールする**(`height:calc(92*var(--vh))` + `display:flex`)。以前は箱ごとスクロールしていたため、**アイテムが増えると説明欄と「使用する」が画面の下へ押し出されて見えなくなっていた**(発注者指摘・2026-08-17)。スクロールするのは`#bagIconGrid`(左)/`#bagTargetList`(右)/`#bagTitleGrid`(称号)の3つだけ。**この3つに`max-height`を書き足さない**(箱の高さと食い違って余白や二重スクロールになる)。
- **右のマスモン一覧は選ぶまで閉じておく**(`renderBagTargetList`)。開くのは選択中の1体だけで、他は見出しの右に1行サマリ(そのアイテムでいちばん知りたい数字)を出す。全部開くと1画面に2体しか入らなかった。
- **選択中の1体は先頭へ持ってきて`position:sticky; top:0`で貼り付ける。** 左のアイテムを切り替えても一覧をスクロールしても位置が動かない。**貼り付ける要素は不透明な背景にする**(半透明のままだと下の行が透ける)。選択中以外は並べ替えない(位置が飛ぶと選び直しになる)。
- **使えない相手は`bagTargetBlockReason(itemKey, mmKey)`1か所で決める。** 一覧の選択可否・使用ボタンの可否・押したときの拒否を全部これで判定する。別々に書くと「一覧では選べるのに押すと断られる」がすぐ起きる。今の理由は 遠征中 / ステータス上限 / (ホラ貝で)遠征中でない の3つ。
...
- **既存のSSR専用メディア再生の仕組みをそのまま流用し、新しい配線を作らない**:
  ...
  - **SEは`gallerySkinSeName(id, slot)`で「実際に試合中に鳴る名前」を解決してから`playSe(name)`。** 解決先は`combat.js`の`SKIN_SUMMON_SE`/`SKIN_TIER3_SE`/`SKIN_HIT_SE`/`SKIN_KILL_SE`/`SKIN_WIN_SE`(**この表が専用SEの唯一の正**。区分の名前と並びは`SKIN_SE_SLOTS`。`SKIN_MEDIA.se`はここへ自動マージされるだけで、`SKIN_MEDIA.se`を直接読んではいけない——古いSSRを見落とす)。`skinSe:id:slot`形式(`SKIN_MEDIA.se`由来)のときだけ`ensureSkinMediaSeBuffers(id)`で追加読み込みが要る。手書きの専用SE(`gokongo`/`rize`等)は`audioInit()`内の`ensureProvidedSeBuffers()`で起動時に読み込み済み。**専用SEが無いスロットは何も表示しない**(共通SEで埋めない。ミュージアムは専用を聴く場所なので、共通SEを鳴らすのは「勝手な仕様変更」として発注者に差し戻された。2026-08-13)。
...
- **時短アイテム(📯帰還のホラ貝)はバッグからも使える**(2026-08-17。以前はバッグでは使えなかった)。対象は遠征に出ている本人なので、バッグのマスモン一覧に**「遠征中の子だけが選べる」状態**で並ぶ。枠を終わらせるのは遠征画面と同じ`expeditionUseRecall()`で、使ったあとは受け取りのために遠征画面へ飛ぶボタン(`showBagJumpBtn('expedition', …)`)を出す。**ガチャのR枠には入れない**(既存のトレチケ・技強化チケットの当たる割合が薄まるため)。
...
# 初回チュートリアル(tutorial.js)

- **順番・文言・完了条件は `data.js` の `TUTORIAL_STEPS` 1つ。** ステップを増やすときはここへ1行足すだけ。`tutorial.js` にステップ名の `switch` を書かない。
- 見せ方は2つだけ: **節目=全画面カード(`#tutorialCard`)/ 操作中=画面下の帯(`#tutorialBand`)+対象に `.tut-spot`**。**穴あきマスクは使わない**(強制横向きの座標変換で位置が合わない)。帯は `pointer-events:none` で下のボタンを覆っても邪魔しない。
- **押せるものを絞るのは capture段のクリック濾過(`tutorialGate`)。** `e.target.closest()` しか使わないので回転しても効く。**こちらから出した合成clickは通す**(`e.isTrusted` で判定)。試合中(`game.started && !game.over`)は濾過しない。
- **必ず1つは押せる**ようにしてある: カードは常に押せる / 帯に入って1.5秒たっても押せる対象が0なら飛ばす / 押せない所を3回叩いても飛ばす。**`allow` を書き忘れると詰む**ので、ステップを足したら `node tools/tutorial_test.mjs` を通す(全ステップで「押せる対象が1つ以上」を検査している)。
- 完了判定は**250msごとに `done()` を見るだけ**。既存のボタンのハンドラは書き換えない。ただし**カードが開いている間は見ない**(先に説明を読ませる)。
- 進捗は `aramon_tutorial_v1`(**端末ごと。`ACCOUNT_SYNC_KEYS` に入れない**)。無料10連の受け取り済みは `aramon_tutorial_gift_v1`。
- **既存アカウントへのログインはローカルのデータを置き換える**(`applyAccountData`)。チュートリアル途中でログインされたら、その場で完了扱いにしてロビーへ返す(`tutorialOnAccount('login')`)。新規作成のときだけ続行する。
- 練習試合の数値は `TUTORIAL_MATCH`(体数・マップの広さ・安置の速さ・botの強さ・botの反応)。**入口の判定は `game.tutorialMatch` 1つ**で、通常の試合の数値は1倍のまま。
