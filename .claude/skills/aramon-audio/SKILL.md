...
- **SSRスキン専用の音(BGM3曲・専用SE5種・昇格演出の音声)は`data.js`の`SKIN_MEDIA`だけが持つ。** audio.jsは表を1周してループ/ワンショットを作るだけなので、**スキンを足してもaudio.jsは1行も増えない**(`skinBgmLoops`/`skinMediaSeOneShots`/`skinPromoteSeOneShots`)。専用SEは`skinSe:<スキンid>:<区分>`という名前で`SE_DEFS`へ入り、combat.jsの`SKIN_SUMMON_SE`/`SKIN_TIER3_SE`等へも自動で登録される(手書きの指定があるスキンはそちらが優先。ちょこの召喚SEは内蔵データURIなので手書きのまま)。**未ロード・取得失敗のときは既存のSE(`SKIN_SE_FALLBACK`)へ落ちる。**
- **SEの区分は`data.js`の`SKIN_SE_SLOTS`が正**(`summon`召喚演出 / `tier3` / `hit` / `kill` / `win`。並びは試合で鳴る順)。**区分を増やすときに触るのは4か所だけ**: `SKIN_SE_SLOTS`(名前と並び)/ audio.jsの`SKIN_SE_FALLBACK`・`SKIN_SE_GAP`(代わりに鳴る音と間引き)/ combat.jsの自動マージの表(差し替え先の表)/ `tools/studio_web.html`の`MEDIA_ITEMS`と書き出しの並び。ギャラリーのミュージアムと管理者画面のSE確認は`SKIN_SE_SLOTS`から作るので追記不要(2026-08-17に`summon`を足したときの実例)。
- **ギャラリーのミュージアムに並ぶ「専用SEがある/ない」の判定は、combat.jsの差し替え表(`SKIN_SUMMON_SE`等)を見る。** `SKIN_MEDIA.se`だけを見ると、内蔵データURIで持っている手書きの専用SE(ちょこ)が漏れる。
