...
- **`SSR_SKIN_TIER3` を直接引かない。必ず `skinTier3Def(skinId)` を通す。** 覚醒スキンには専用技の行が無い(ツールが作るのは見た目だけ)ので、直接引くと**技名が通常の名前に戻り、SSRの威力倍率まで失われる**(実機で報告・2026-08-11)。`skinTier3Def` が `awakenOf` をたどって元のスキンの定義を受け継ぐ。
- 覚醒スキンに独自の技名を付けたいときは `SSR_SKIN_TIER3` にその覚醒IDの行を足せばよい(あればそちらが優先される)。
