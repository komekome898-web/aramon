...
- **触ったら `node tools/lobby_layout_test.mjs`。** 7端末(縦持ち4・横持ち2・タブレット)×
  5モード × モンスター選択の有無 × 負荷3種(素 / ボタン+9個 / 文字を全部長く)= 210通りで、
  見切れ0 / 押しやすさの下限割れ0 / **文字切れ0** / 重なり0 / スクロール0 を確かめる。
...
## 見えるスクロールバー(自前・全欄に自動で付く)

**iOSのSafariは`::-webkit-scrollbar`を描かない**(指で動かしている間だけ細い線が出て離すと消える)。
そのためCSSでは常時表示のバーを作れないので、`ui.js`の`attachVisibleScrollbar(el, bar)`が自前で1本描く。**新しいスクロールバーの実装を作らない。**

- **どこがスクロール欄かは`style.css`から集める**(`scrollAreaSelector()`)。`overflow`/`overflow-y`を`auto`/`scroll`にしている規則のセレクタを起動時に拾うので、**CSSに欄を1つ足せばバーもついてくる**(画面ごとの対応表を作らない)。`::`や`&`を含むセレクタは`querySelectorAll`に渡せないので捨てる(**1本混ざると全滅する**)。
- **`r.cssRules`があるからといって入れ子とは限らない。** CSSネスト対応のブラウザでは普通のスタイル規則にも空の`cssRules`が付く(truthy)。「入れ子だから飛ばす」と書くと**1件も拾えない**(実際にそうなった)。
- **空の結果を覚えない。** 起動直後はCSSが読み終わっていないことがある。空をキャッシュすると以降ずっとバーが付かない。
- バーは**スクロール欄の中**に入れる(絶対配置なのでgrid/flexの並びには参加しない)。位置は`scrollTop`ぶん置き直す。**`innerHTML`で中身を作り直すとバーごと消えるので、sweepで同じ要素を戻す**(作り直すとスクロール監視が二重に付く)。
- **溝は`pointer-events:none`、つまみだけ`auto`。** 6pxの帯でも、下のボタンの右端が「押しても何も起きない」死んだ場所になる。
- 手でHTMLに置いてあるバー(マスモン詳細など)には二重に付けない。`el._scrollbarUpdate`と`親 > .mm-scrollbar`の両方を見る(監視が付くのは画面を組んだ時なので、sweepが先に走ると2本になる)。
- **つまみのドラッグは両方の軸を`toLogicalDelta`へ渡す。** 強制横向きでは画面の「下」は実座標のx方向なので、`toLogicalDelta(0, dy).y`は必ず0になり、**縦持ちではつまみを掴んでも1pxも動かなかった**(2026-08-17に修正)。
