...
- **【山肌の塗り分けを「面ごと」に決めてはいけない】** 山の面は主峰で1枚160単位もある。面の重心で「この面は岩・この面は雪」と決めると必ずブロック塀のようなパッチワーク(迷彩柄)になる。実機で2度reportされ、原因を替えて4度作り直した末の結論(2026-08-14)。踏んだ罠を全部残す:
  - **面の法線** — 四角形は三角形2枚で、曲面では2枚の法線が違う → 三角形1枚おきの市松。
  - **細かいノイズ** — `grit`(48スポーク)・まだら(19〜62単位周期)は分割数(周64/36・縦15/9)と干渉して面ごとの乱数になる。
  - **粗いノイズ1枚** — 9スポーク1枚だと`tileNoise`の整数格子がそのまま出て規則正しい菱形になる。
  - 正解は **頂点属性`aCoat`(頂点ごと=ラスタライザが面をまたいで補間)+ 画素ごとの法線マップのゆらぎ**。`applyMountainCoat()`。`mountainRib()`は5と7の2オクターブ+ドメインワープで「面より粗く格子に見えない」帯に収めてある。**オクターブを増やして細かくしないこと。**
- **雪山は「白が地」。** `foot/mid/top`を3色とも雪の色にし、急な尾根の背にだけ`rock`を出す。露岩は太陽4.6+ACESでも沈むよう十分暗く(`themeColor('steep')`の混ぜは0.15まで。0.40混ぜたら`#546172`まで明るくなって雪と区別が付かなくなった)。
- **山だけシーンの霧を切って自前で薄く掛ける**(`MOUNT_FOG_NEAR/FAR`)。地形パッチ用の`FOG_FAR=3200`は半径1500・高さ1900の山には近すぎ、隣に立っていても山肌の8割が霞の色へ溶ける(カウレアの黒い火山が砂色に見えていた真犯人)。**threeの霧uniformは毎フレームrendererがシーンから書き戻すので、uniformの差し替えでは効かない。**
- **2Dで世界に描く物を足したら必ず`occludedByMountain()`を通す。** モンスター・弾・アイテムは通していたのに技エフェクト(`areaEffects`)とパーティクルだけ素通りで、山の裏の技が透けて見えていた(2026-08-14)。技は根元・中間・先端の3点すべてが山の中のときだけ消す(`areaEffectOccluded()`)。着弾マーカーも同様。
...
- **【当たり判定の無い物を「隠れられそうな物」の形にしない】** 飾りは草・枝・葉だけ。丸い塊・板・柱の形にしない。2つ廃止した(2026-08-14):
  - 飾りの小石(`rockSilhouette()`で作っていた。数百個)
  - 雪・火山灰の塊(`kind:'lump'`。`boulderGeo()`+障害物と同じ`applySurfaceDetail`。**画面上の岩に見える物の大半がこれだった**)
