...
const UPDATE_HISTORY = [
  { date:'2026-07-26', items:[
    'マスモンの選択画面も刷新しました。モンスター一覧と同じようにカードを左右にスワイプして選べるようになり、カードをタップすると詳細情報・技一覧・トレーニング・着せ替え・編集をその場で切り替えられます',
    'カードの詳細画面で、カードの絵の両サイドにある「≪ ≫」ボタンから隣のモンスター/マスモンへ直接移動できるようにしました',
    'モンスター一覧の画面を刷新しました。カードを左右にスワイプして選べるようになり、中央のカードが大きく表示されます(端まで行くと最初に戻るのでぐるぐる回せます)。カードをタップすると、そのモンスターのステータス・技・状態変化をまとめて確認して、その場で「このモンスターで参戦」できます',
    'マルチプレイでもマスモンの育成ステータスが反映されるようになりました(ホスト・ゲストのどちらで参加しても、自分と相手の全員に育成した強さが乗ります)',
    'マスモンの撃破ボーナス(経験値)が、マスモンbotだけでなく相手プレイヤーのマスモンを倒したときにも入るようになりました。ホスト・ゲストのどちらでも受け取れます',
    'マルチプレイのゲスト側で安全圏の縮小までの残り時間が進まなかったのを修正(縮小に備えられるようになりました)',
    'マルチプレイのゲスト側で状態変化(暴走・我慢など)の発動が分からなかったのを修正。やけど・どくの表示も出るようになりました',
    'マルチプレイのゲスト側で連射する範囲技が1発しか出ていなかったのと、自分で撃ったビッグバン/ヴァニッシュの爆風が見えなかったのを修正',
    'マルチプレイのゲスト側で安全圏外・溶岩のダメージ表示が出なかったのを修正(HPが減る理由が分かるようになりました)',
    'マルチプレイで凍結中に攻撃できるかどうかがホストとゲストで違っていたのを統一(凍結中は攻撃できません)',
    ...
    'マルチプレイのゲスト側でダッシュしても元の位置に戻されるのを修正',
    ...
    'マルチプレイの観戦画面で「次のプレイヤー」を押しても視点が変わらないことがある問題を修正(生き残っているモンスターを順番に見られます)',
  ...
  { date:'2026-07-24', items:[
    ...
    '更新履歴画面を追加',
    '敵AIを強化（ガッツ切れ時にガッツ飴を探す・障害物で詰まったら迂回・プレイヤーのマスモンレベルに応じた強さ）',
    'マルチプレイに観戦機能を追加（撃破後も生き残りプレイヤーを観戦）',
    'ガリのSSRスキン「ゼウス」を追加（tier3専用エフェクト＆SE）',
    'ショップに店主「ラガゼウスモチ」を追加（購入でセリフが変化）',
    '全画像を軽量化し、読み込みを高速化',
  ]},
  { date:'2026-07-23', items:[
    'オーラ相性システムを導入（相性でダメージが増減・オーラ一致でボーナス）',
    'オートラン機能を追加（ジョイスティックを上に2回はじくと視点方向へ前進し続ける）',
    'バトル操作画面のカスタマイズ機能を追加（各パーツの配置・サイズを変更して記憶）',
    'マルチプレイのプレイ感を改善（位置補間・ラグ補正で相手の動きを滑らかに）',
    'モッチーのSSRスキン「ラガモッチー」を追加（シーズンパス限定）',
  ]},
  { date:'2026-07-22', items:[
    '称号システムを追加（実績で解放し、最大3つ装着）',
    'デイリー機能を追加（ログインボーナス＋毎日のミッション）',
    'シーズンパスを追加（試合でSPを貯めて25段階の報酬を獲得）',
    '新モンスター「ガリ」「ザン」を追加',
    'SSRスキン「タマモノマエ」「イブリース」を追加',
    'SSR獲得演出を追加',
  ]},
  { date:'2026-07-21', items:[
    'モンスター着せ替え（スキン）機能とスキンガチャを追加',
    '試合開始の召喚演出を追加',
    'マルチプレイのスキン反映・障害物同期を改善',
  ]},
];
