...
/* ジョーカーはザンと同じ見せ方を使う(tier2は同じ三日月、tier3は同じ黒い刃の鎌)。
   **既定(FX_DEFAULT)のままだと明るい光の球が中央に出て、黒い刃が白い塊に見える。**
   ザンの表は「中を塗らず縁だけ細く光らせる」= 黒が黒く見える作りなのでそのまま流用する。 */
FX_MOVES.joker = FX_MOVES.zan;
...
/* ---- ケンタウロス(草): メテオドライブ ----
   **ピクシーと同じ表をそのまま使う**(発注者指定・2026-08-17)。吸い込む球・二重の輪・
   煤の尾という演出はそのままに、色は c(=技の色=緑)から作られるので緑のまま出る。
   写しを作らず参照を張るので、ピクシー側を直せばこちらも一緒に直る
   (手書きの対応表を新しく作らない、の原則)。性能値は data.js 側でケンタウロスのまま。 */
FX_MOVES.centaur = FX_MOVES.pixie;
