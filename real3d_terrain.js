...
const TEX_STYLES = {
  ...
  dry: {
    ...
    make(){
      const tc = wtable(20,20, 3), tp = wtable(34,34, 11);
      ...
      return (u,v,px,py,out)=>{
        ...
        worley(u*34 + wu*2.6, v*34 + wv*2.6, 34, 34, tp, p);
        ...
        // 石の大きさを1つずつ変える。同じ大きさで並ぶと水玉模様になる
        const peb = Math.pow(Math.max(0, 1 - p[0]/(0.20+pid*0.34)), 0.6) * on;
        const g = ih(px*3+1, py*7+2);
        out[0] = 0.80 + (soil-0.5)*0.24 + peb*(0.06+pid*0.20) - crack*0.15 + (g-0.5)*0.11;
        out[1] = (soil-0.5)*0.8 + (pid-0.5)*1.3*on;
        out[2] = peb*1.00 + soil*0.30 - crack*0.70 + g*0.10;
      };
    },
  },
  ...
  volcanic: {
    ...
    make(){
      ...
      return (u,v,px,py,out)=>{
        ...
        const pit = Math.max(0, 1 - p[0]/0.34) * (ih(p[2]+3, 29) > 0.30 ? 1 : 0);
        const g = ih(px*5+7, py*3+11);
        out[0] = 0.76 + (ash-0.5)*0.26 + (cid-0.5)*0.17 - crack*0.26 - pit*0.18 + (g-0.5)*0.14;
        out[1] = (ash-0.5)*1.0 + (cid-0.5)*0.6 - crack*0.4;
        out[2] = ash*0.40 + (cid-0.5)*0.35 - crack*0.95 - pit*0.65 + g*0.14;
      };
    },
  },
  // 砂: 風紋が主役。ひび割れは作らない
  ...
  sand: {
    ...
    make(){
      ...
      return (u,v,px,py,out)=>{
        ...
        const peb = Math.max(0, 1 - p[0]/(0.14+pid*0.16)) * (pid > 0.80 ? 1 : 0);
        const g = ih(px*5+3, py*3+9);
        out[0] = 0.86 + (rip-0.45)*0.13 + (big-0.5)*0.055 + (g-0.5)*0.075 + peb*0.09;
        out[1] = (big-0.5)*0.7 + (rip-0.5)*0.35 + peb*0.8;
        out[2] = rip*0.80 + big*0.28 + g*0.12 + peb*0.6;
      };
    },
  },
  ...
  jungle: {
    ...
    make(){
      ...
      return (u,v,px,py,out)=>{
        ...
        const leaf = Math.pow(Math.max(0, 1 - l[0]/(0.13+lid*0.55)), 0.8) * (lid > 1 - pile ? 1 : 0);
        const g = ih(px*3+13, py*5+1);
        out[0] = 0.90 + (moss-0.5)*0.24 + leaf*(0.10 + lid*0.16) - (1-leaf)*0.08 + (g-0.5)*0.12;
        out[1] = leaf*(0.35 + lid*1.2) - (1-leaf)*(0.45 + moss*0.55);
        out[2] = leaf*0.85 + moss*0.32 + pile*0.20 + g*0.12;
      };
    },
  },
};
...
function buildGroundMaps(st){
  ...
  for(let i=0;i<N;i++) hgt[i] = (hgt[i]-hMin)*hs;
