...
// 残り続け、更新した内容が反映されません。
const CACHE_NAME = 'aramon-cache-v782';
// 画像と音は「別のキャッシュ」に入れ、バージョンを上げても消さない。
// コード(html/js/css)だけが毎回入れ替わり、11MBの画像と5.7MBの音は貯めたまま使える。
const MEDIA_CACHE = 'aramon-media';
/* スタジオの背景抜きモデル(約44MB)のキャッシュは 2026-09-04 に機能ごと廃止した。
   端末に残っている古いキャッシュは、下の activate が「知らないキャッシュ」として消す
   (除外リストから外してあるので、次にアプリを開いた時点で44MBが解放される)。 */
const CORE_ASSETS = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      // 画像・音のキャッシュ(MEDIA_CACHE)だけは世代交代で消さない
      Promise.all(keys.filter((k) => k !== CACHE_NAME && k !== MEDIA_CACHE)
