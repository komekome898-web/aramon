...
- **形の定義は`data.js`の`OBST_SHAPES`1か所**(`h`全高 / `sink`地面へ埋める深さ / `sil`くり抜く楕円)。real3d.jsは`window.__aramonObstShapes`、render.jsは同名の定数として同じ表を読む。**3Dモデルの寸法を変えたら必ず表も直す**(ずれるとモンスターが障害物の縁に乗る/欠ける)。単位は当たり判定の半径=1。
- **くり抜きは少し大きめ(`OBST_ERASE_PAD`)。** リアルマップの2Dは地面を描いていないので、大きすぎても下のWebGLの地面が見えるだけで害はない。小さいと奥のモンスターが縁からはみ出す。
- **高さは決め打ちで縮尺せず`project(x,y,地面+r*h)`で実測する。** 背の高い木ほど遠近の差が出る。
