...
3. **ビルドツール・npmパッケージ・フレームワークを導入しない。** すべて素のJS/CSS/HTMLのまま維持する。
   **例外(発注者判断・2026-07-28)**: 「リアルマップ(テスト)」だけThree.jsを使う。`vendor/three.module.min.js`にファイルを同梱しているだけで、package.jsonもnode_modulesもビルド手順も無い(素の`<script type="module">`で読む)。この例外を他の機能へ広げないこと。
...
| `real3d.js` | 「リアルマップ(テスト)」のWebGL描画(ESモジュール。`window.__aramonReal3D`で公開) |
| `vendor/three.module.min.js` | Three.js r160(MITライセンス。`vendor/three-LICENSE.txt`)。npm依存ではなくファイル同梱 |
...
- **【メディアクエリ禁止・全画面共通】レイアウトに`@media`を使ってはいけない。** 幅で分岐したいときはworld.jsが論理幅で付ける`html.narrow-screen`(実画面の幅520px以下。旧`@media (max-width:520px)`と同条件)を使う。
...
### リアルマップ(テスト): WebGLの立体地形(real3d.js / vendor/)
- **地面だけをWebGL(Three.js)で描き、モンスター・弾・技エフェクト・HUDは従来の2Dキャンバスが上に重ねる。** この分担なので既存の描画コードを書き換えずに済んでいる。`#glCanvas`(z-index:0)の上に`#gameCanvas`(z-index:1)。
- **2Dの`project()`と3Dカメラを完全に一致させてある**(縦画角64°=`FOV_V` / カメラ位置=`camPos` / `camState.yaw`・`pitch`)。だから2Dが描く物の画面位置は地形とズレない。**`FOV_V`や`CAM_*`を変えたら`real3d.js`のカメラ設定も合わせること。** 丘の裏に隠れる遮蔽は2D側に無い(テストマップの割り切り)。
- **地形の高さは`data.js`の`real3dHeightAt(x,y)`。純関数なのでホスト/ゲストで自動的に一致する**(シード付き乱数を通す必要がない)。当たり判定(`world.js`の`getTerrainHeightAt`)も同じ関数を使うので、見た目と当たり判定が必ず一致する。
- **`REAL3D_TERRAIN`の「振幅×周波数」の合計=最大傾斜を0.3程度に抑える。** ダッシュは1フレームで20単位ほど進むので、傾斜が大きすぎると高さの差が`CLIMB_TOLERANCE`(12)を超えて坂を登れなくなる。
- **岩・水晶の「登っているからすり抜ける」判定は`baseTerrainHeightAt`基準にする。** 絶対値の`m.z > 25`だと、起伏でzが上がっただけで岩をすり抜けてしまう(実際に踏みかけた)。
- **地面に接する物は`projectGround(x,y)`で投影する。** `project(x,y,0)`のままだと起伏のある地形で浮いて見える(円盤石・アイテム・岩・水晶・範囲技・安全圏・爆風・落下地点マーカーが該当し、実際に浮いた)。`groundZAt()`は他マップでは0を返すので見た目は変わらない。**新しく地面に接する物を描くときは必ず`projectGround`を使う。**
- **エンティティに紐づく地面の描画は`e.z`を使う**(召喚円盤石・降下ビーム)。エンティティは既に地形の高さを持っているので0を渡してはいけない。
- **地面の細かい質感はテクスチャで出し、メッシュには入れない。** メッシュの分割は約50単位なので、それより細かい波長の起伏を`REAL3D_TERRAIN`に足してもジャギーになるだけ。`buildGroundTexture()`が値ノイズ4オクターブのタイルを生成して貼る。
- **地面テクスチャのUVオフセットをパッチ位置に合わせること**(`tex.offset.set(sx/TEX_TILE, -sy/TEX_TILE)`)。これが無いと模様がパッチと一緒に動き、地面の上を滑って見える。`uv.y`は`rotateX(-π/2)`で反転しているので符号が逆になる。
- **`ESモジュール`なので`window.__aramonReal3D`経由で呼ぶ**(firebase.jsと同じ橋渡し)。`setActive(bool)`/`render()`/`resize()`。WebGL初期化に失敗した端末では`render()`がfalseを返し、render.jsが従来の2D地面を描くので壊れない。
- テスト用マップは`MAPS`に`testOnly:true`を付けて「ランダム」の抽選対象から外している。
...
- **幅による分岐は`html.narrow-screen`**(world.jsが実画面の幅520px以下で付ける。旧`@media (max-width:520px)`と同条件)。`@media`をやめてクラスにしておけば、強制横向きで基準が食い違う問題に巻き込まれず、JS側から意図した基準を選べる。
...
- **向きの判定は`matchMedia('(orientation: portrait)')`で行う。** 実測pxは起動直後に確定していないことがあり、その値で一度だけ判定すると「縦持ちで起動したときだけ効かない」状態で固定される。`getRealViewportSize()`も`visualViewport`→`innerWidth/Height`→`clientWidth/Height`の順にフォールバックする。
- **起動直後は`resize()`を何度も呼び直す**(`DOMContentLoaded`/`load`/`pageshow` + 50〜2000msのタイマー + メディアクエリのchange)。iOSは`orientationchange`を取りこぼすことがある。`resize()`のキャンバス処理はtry/catchで囲み、失敗しても向きの判定だけは済んだ状態にする。
