...
- **遊び心地の確認はiPhone実機(PWA)で発注者が行う。** こちらは`node --check <file>`の構文チェックに加え、**見え方・不具合の再現・重さの比較はヘッドレスで本物のゲームループを回して確かめる**(ループを止めて手でコマを進める道具では確かめたことにならない。2026-09-25 探検の敵の透け)。**画面の実測はヘッドレスで行う**(`tools/measure_layout.mjs`。スクロール量・見切れ・持ち方での文字サイズ差を数字で出す。レイアウトを触ったら前後で測って報告する)。
