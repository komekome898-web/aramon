...
- **ビーム系(モッチ砲/ラガモッチ砲/天河天翔/熱視線/フラワービーム)は`fx3dBeamTube`だけで描き、違うのは色だけにする。** 断面の半径=**当たり幅の半分**なので真後ろから見ると円になる。膨らませる向きは**画面上の進行方向に対して垂直**(画面の上下に固定すると坂の下へ撃った時にねじれる)。両端に断面の円を足す。
...
- **遠くのアイテムは粒だけで描く**(`LOOT_SIMPLE_SCALE`)。試合開始直後はアイテムが数十個あり、1個ずつ楕円・矩形・文字を重ねると効く。**通常時のしきい値は「画面上で数pxしかない距離」まで下げる**(0.42にしたら近くのアイテムまで粒になった)。重いときだけ`LOOT_SIMPLE_SCALE_HEAVY`で早めに切り替える。
