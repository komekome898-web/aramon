...
- **ビーム系(モッチ砲/ラガモッチ砲/天河天翔/熱視線/フラワービーム)は`fx3dBeamTube`だけで描き、違うのは色だけにする。** 断面は**「横=技の当たり幅 / 縦=`FX3D_MON_H`」の楕円**(真円にすると幅の広い技で縦にも同じだけ塗ることになり、塗る面積がほぼ倍になる)。断面は`uH`(横)と`uV`(縦)の2ベクトルを実際に投影して持ち、輪郭の膨らみは`hypot(n・uH, n・uV)`で厳密に出す。両端の断面はこの2ベクトルを基底に`ctx.transform`して真円を描けば、投影された楕円がそのまま出る。
